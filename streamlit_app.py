...
def _get_client_with_retry():
    """Get the agent client, clearing cache and retrying on auth failure."""
    client = get_agent_client()
    # Force a fresh token on each request to avoid stale JWT issues
    client._token = None
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            try:
                ...
                error_str = str(e)
                if "401" in error_str or "auth" in error_str.lower() or "JWT" in error_str:
                    # Clear the cached client and try once more with a fresh token
                    get_agent_client.clear()
                    try:
                        client = _get_client_with_retry()
                        _show_status("Retrying with fresh token...")
                        response, thread_id, message_id = process_query_v2(
                            client, prompt,
                            st.session_state.thread_id,
                            st.session_state.parent_message_id,
                        ...
                        response = (
                            ...
                            "(HTTP 401) even after refreshing the token.\n\n"
                            "This usually means:\n"
                            "- The RSA public key fingerprint doesn't match what's registered on the Snowflake user\n"
                            "- The account identifier format is wrong (should be `orgname-accountname`, e.g. `xlbpipm-il76509`)\n"
                            "- The private key in secrets.toml doesn't correspond to the public key set on the user\n\n"
