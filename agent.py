    ...
    def _parse_sql_api_response(self, data: dict, prev_thread_id: int = None) -> dict:
        ...
        if not result["text"]:
            result["text"] = (
                ...
                "or my query didn't return results.\n\n"
                "I can help with: population, income, housing, education, employment, "
                "poverty, commuting, health insurance, internet, language, household types, "
                "veterans, and SNAP — at state or county level (ACS 2020)."
            )
...
def process_query_v2(client: CortexAgentClient, user_message: str,
                     thread_id: int = None, parent_message_id: int = None,
                     conversation_history: list = None) -> tuple:
    """
    Process a user query via the Cortex Agent REST API.

    Validation pipeline:
      1. Input sanitization (injection, length) — fast, no LLM call
      2. LLM topic classification — only if stage 1 passes
      3. Cortex Agent execution — only if stage 2 passes
    """
    # --- Stage 1: Input sanitization (no network, instant) ---
    sanitize_result = sanitize_input(user_message)
    if sanitize_result.result == ValidationResult.BLOCK:
        return sanitize_result.message, thread_id, None

    # --- Stage 2: Full input validation (keyword fast-path + LLM classifier) ---
    guard_result = validate_input(user_message, conversation_history)
    if guard_result.result == ValidationResult.BLOCK:
        return guard_result.message, thread_id, None

    # Stage 2b: LLM classifier (only for WARN or new conversations without keywords)
    is_on_topic, refusal = _classify_topic(client, user_message, conversation_history)
    if not is_on_topic:
...
- Questions about population, income, housing, education, employment, poverty, commuting, health insurance, internet access, language, household types, veterans, SNAP/food stamps, marital status, school enrollment, earnings, occupation, geographic mobility
- Questions about US states, counties, or demographic comparisons
- Follow-up questions that reference prior census data answers (e.g. "what about Texas?", "show me the lowest", "and the top 5?", "compare that to California")
- Short replies that continue an ongoing census conversation (e.g. "yes", "county level", "top 10")
...
IMPORTANT: If conversation context is provided, use it to determine intent. A short or ambiguous message that follows a census-related conversation should be treated as ON-TOPIC.
...
def _classify_topic(client: CortexAgentClient, user_message: str,
                    conversation_history: list = None) -> tuple:
    ...
    # Build prompt with conversation context for follow-up awareness
    prompt_parts = [CLASSIFIER_PROMPT]
    if conversation_history:
        ...
        prompt_parts.append("")
    prompt_parts.append(f"User message to classify: {user_message}")
    prompt = "\n".join(prompt_parts)
